...
public class Paginator implements java.io.Serializable {
    ...
    public Paginator(int page, int rows, int total) {
        super();
        this.rows = rows;
        this.total = total;
    ...
    public int getStartRow() {
        if (getRows() <= 0 || total <= 0)
            return 0;
        return page > 0 ? (page - 1) * getRows() + 1 : 0;
    }
    ...
    public int getEndRow() {
        return page > 0 ? Math.min(rows * page, getTotal()) : 0;
    }
    ...
    public int getOffset() {
        return page > 0 ? (page - 1) * getRows() : 0;
    }
    ...
    public int getLimit() {
        if (page > 0) {
            return Math.min(rows * page, getTotal()) - (rows * (page - 1));
        } else {
            return 0;
        }
    }
    ...
    protected int computePageNo(int page) {
        return computePageNumber(page, rows, total);
    }
    ...
    public String toString() {
        return "page:" + page + " pageSize:" + rows + " totalItems:" + total;
    }
